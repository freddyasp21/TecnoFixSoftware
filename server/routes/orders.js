...
router.get('/lookups/technicians', requirePermission('orders.view'), (_req, res) => {
  const rows = getDb().prepare(`
    ...
    WHERE u.active = 1 AND r.name IN ('Tecnico', 'Administrador')
...
router.get('/', requirePermission('orders.view'), (req, res) => {
  ...
  let sql = `
    SELECT wo.*, c.name AS client_name, t.full_name AS technician_name, q.number AS quote_number
    FROM work_orders wo
  ...
  if (q) {
    sql += ' AND (wo.number LIKE ? OR c.name LIKE ? OR wo.serial_number LIKE ? OR wo.device_model LIKE ?)';
    const like = `%${q}%`;
...
router.put('/:id', requirePermission('orders.manage'), (req, res) => {
  ...
  try {
    db.transaction(() => {
      ...
      if (b.status === 'listo' && !readyAt) readyAt = new Date().toISOString().replace('T', ' ').slice(0, 19);
      if (b.status === 'entregado' && !deliveredAt) deliveredAt = new Date().toISOString().replace('T', ' ').slice(0, 19);
      ...
      db.prepare(`
        ...
        WHERE id = ?
      `).run(
        b.client_id ?? null, b.technician_id ?? null, b.status ?? null,
        ...
        ivaEnabled, ivaRate, readyAt, deliveredAt, order.id
      );
